...
int main()
{
	StreamClient streamClient("localhost", "50012");
	cout << "Client instantiated\n";
	cout << "Connection : " << streamClient.connectStream() << "\n";



	while (streamClient.isConnected) {
		if (streamClient.readStream() != 0)
		{
			// if reading failed : reconnect
			cout << "reconnected\n";
