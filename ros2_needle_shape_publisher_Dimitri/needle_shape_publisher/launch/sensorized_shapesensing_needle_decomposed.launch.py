...
def generate_launch_description():
    ...
    # arguments
    arg_needleparam = DeclareLaunchArgument(
        'needleParamFile',
    ...
    node_sensorizedneedle = Node(
        ...
        emulate_tty=True,
        parameters=[{
            'needle.paramFile'                   : LaunchConfiguration( arg_needleparam.name ),
            'needle.numberSignals'               : LaunchConfiguration( arg_numsignals.name ),
            'needle.sensor.temperatureCompensate': LaunchConfiguration( arg_temp_compensate.name ),
        }]
    )
    node_ssneedle = Node(
        package='needle_shape_publisher',
        ...
        parameters=[{
            'needle.paramFile'                               : LaunchConfiguration( arg_needleparam.name ),
            'needle.optimizer.max_iterations'                : LaunchConfiguration( arg_optim_maxiter.name ),
            'needle.optimizer.update_orientation_with_airgap': LaunchConfiguration( arg_optim_update_ornt_airgap.name ),
        }],
    )
