...
class ShapeSensingNeedleNode( NeedleNode ):
    ...
    def __init__( self, name="ShapeSensingNeedle" ):
        ...
        pd_optim_maxiter = ParameterDescriptor(
            name=self.PARAM_OPTIM_MAXITER,
            type=Parameter.Type.INTEGER.value,
            description="Maximum iterations for convergence"
        )
        optim_maxiter = self.declare_parameter(
            pd_optim_maxiter.name,
            value=15,
            descriptor=pd_optim_maxiter
    ...
    @property
    def inserted_into_tissue( self ):
        return self.current_needle_pose[0][2] >= self.ss_needle.insertion_point[2]
    ...
    def get_needleshape( self ):
        ...
        L_needle      = (
            utilities.calculate_needle_length(pmat)
            if (pmat is not None) and self.inserted_into_tissue
            else 0
        )
        dL            = self.ss_needle.length - L_needle
        pmat_straight = np.zeros((1, 3), dtype=np.float64)
        if dL > self.ss_needle.ds:
            ...
            pmat_straight       = np.zeros( (len(L_straight), 3), dtype=pmat_straight.dtype )
            pmat_straight[:, 2] = L_straight

        # if
        elif dL > 0:  # less than ds increment
            pmat_straight        = np.zeros((2, 3), dtype=pmat_straight.dtype)
        ...
        if (pmat is not None) and (Rmat is not None) and self.inserted_into_tissue:
